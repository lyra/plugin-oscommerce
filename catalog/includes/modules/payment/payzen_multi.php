...
include_once(DIR_FS_CATALOG . "includes/languages/$language/modules/payment/payzen_multi.php");

/**
 * Main class implementing PayZen multiple payment module for OSC.
 */
class payzen_multi {
	/**
	 * @var string
	...
	function payzen_multi() {
		...
		// initialize enabled
		$this->enabled = (MODULE_PAYMENT_PAYZEN_MULTI_STATUS == '1');

		// initialize sort_order
		$this->sort_order = MODULE_PAYMENT_PAYZEN_MULTI_SORT_ORDER;

		$this->form_action_url = MODULE_PAYMENT_PAYZEN_MULTI_PLATFORM_URL;

		if ((int)MODULE_PAYMENT_PAYZEN_MULTI_ORDER_STATUS > 0) {
			$this->order_status = MODULE_PAYMENT_PAYZEN_MULTI_ORDER_STATUS;
		}
	...
	function update_status() {
		...
		// check customer zone
		if ((int)MODULE_PAYMENT_PAYZEN_MULTI_ZONE > 0) {
			$flag = false;
			$check_query = tep_db_query('SELECT `zone_id` FROM `' . TABLE_ZONES_TO_GEO_ZONES . '`' .
										" WHERE `geo_zone_id` = '" . MODULE_PAYMENT_PAYZEN_MULTI_ZONE . "'" .
		...
		if ((MODULE_PAYMENT_PAYZEN_MULTI_AMOUNT_MIN != '' && $order->info['total'] < MODULE_PAYMENT_PAYZEN_MULTI_AMOUNT_MIN)
				|| (MODULE_PAYMENT_PAYZEN_MULTI_AMOUNT_MAX != '' && $order->info['total'] > MODULE_PAYMENT_PAYZEN_MULTI_AMOUNT_MAX)) {
			$this->enabled = false;
		}

		// check currency
		$payzenMultiApi = new PayzenMultiApi('UTF-8'); // load PayZen payment API
		...
		if(!$payzenMultiApi->findCurrencyByAlphaCode($order->info['currency']) && !$payzenMultiApi->findCurrencyByAlphaCode($defaultCurrency)) {
	...
	function selection() {
		...
		foreach ($this->get_available_options() as $code => $option) {
			...
			$selection['fields'][] = array(
					'title' => '',
					'field' => '<input type="radio" id="payzen_option_' . $code . '" name="payzen_option" value="'.$code.'" onclick="$(\'input[name=payment][value=payzen_multi]\').attr(\'checked\', true);" style="vertical-align: middle; margin-top: 0;"' . $checked . '>' .
	...
	function process_button() {
		...
		$payzenMultiApi = new PayzenMultiApi('UTF-8');
		...
		foreach ($configParams as $name) {
			$payzenMultiApi->set($name, constant('MODULE_PAYMENT_PAYZEN_MULTI_' . strtoupper($name)));
		}
		...
		$payzenLanguage = $payzenMultiApi->isSupportedLanguage($langData['code']) ?
		...
		$payzenCurrency = $payzenMultiApi->findCurrencyByAlphaCode($order->info['currency']);
		if(!$payzenCurrency) {
			...
			$payzenCurrency = $payzenMultiApi->findCurrencyByAlphaCode($defaultCurrency);
			$currencyValue = 1;
		}
		...
		// activate 3ds ?
		$threedsMpi = null;
		if(MODULE_PAYMENT_PAYZEN_MULTI_3DS_MIN_AMOUNT != '' && $order->info['total'] < MODULE_PAYMENT_PAYZEN_MULTI_3DS_MIN_AMOUNT) {
			$threedsMpi = '2';
		}
		...
		$first = (key_exists('first', $option) && $option['first'] != '') ?
				$payzenCurrency->convertAmountToInteger(($option['first'] / 100) * $total) :
				NULL;

		// override cb contract
		$data['contracts'] = $option['contract'] ? 'CB=' . $option['contract'] : null;

		$payzenMultiApi->setFromArray($data);
		$payzenMultiApi->setMultiPayment(NULL /* use already set amount */, $first, $option['count'], $option['period']);

		return $payzenMultiApi->getRequestFieldsHtml();
	}
	...
	function before_process() {
		global $order, $payzenMultiResponse, $messageStack;
		...
		$fromServer = $payzenMultiResponse->get('hash');
		...
		if(MODULE_PAYMENT_PAYZEN_MULTI_CTX_MODE == 'TEST') {
		...
		if($payzenMultiResponse->isAcceptedPayment()) {
			...
			if($this->_is_order_paid()) {
				...
				$order->info['cc_type'] = $payzenMultiResponse->get('card_brand');
				$order->info['cc_number'] = $payzenMultiResponse->get('card_number');
				$order->info['cc_expires'] = str_pad($payzenMultiResponse->get('expiry_month'), 2, '0', STR_PAD_LEFT) . substr($payzenMultiResponse->get('expiry_year'), 2);

				// use payment method short name to avoid truncate
				$order->info['payment_method'] = MODULE_PAYMENT_PAYZEN_MULTI_SHORT_TITLE;

				// let's borrow the cc_owner field to store transaction id
				$order->info['cc_owner'] = '-&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Transaction: ' . $payzenMultiResponse->get('trans_id');
	...
	function after_process() {
		global $cart, $payzenMultiResponse, $messageStack;
	...
	function check() {
		if (!isset($this->_check)) {
			$check_query = tep_db_query("SELECT `configuration_value` FROM `" . TABLE_CONFIGURATION . "`" .
										" WHERE `configuration_key` = 'MODULE_PAYMENT_PAYZEN_MULTI_STATUS'");
	...
	function _install_query($key, $value, $sort_order, $set_function=null, $use_function=null) {
		...
		$sql_data = array(
				...
				'configuration_key' => $prefix . 'MULTI_' . $key,
	 ...
	 */
	function install() {
		// Ex: _install_query($key, $value, $sort_order, $set_function=null, $use_function=null)
		...
		$this->_install_query('OPTIONS', '', 40, 'payzen_cfg_draw_table_multi_options(', 'payzen_get_multi_options');

		// gateway return parameters
		$this->_install_query('REDIRECT_ENABLED', '0', 50, 'payzen_cfg_draw_pull_down_bools(', 'payzen_get_bool_title');
		$this->_install_query('REDIRECT_SUCCESS_TIMEOUT', '5', 51);
		$this->_install_query('REDIRECT_SUCCESS_MESSAGE', 'Redirection vers la boutique dans quelques instants...', 52);
		$this->_install_query('REDIRECT_ERROR_TIMEOUT', '5', 53);
		$this->_install_query('REDIRECT_ERROR_MESSAGE', 'Redirection vers la boutique dans quelques instants...', 54);
		$this->_install_query('RETURN_MODE', 'GET', 55, "tep_cfg_select_option(array(\'GET\', \'POST\'), ");
		$this->_install_query('ORDER_STATUS', '0', 56, 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name');
	}
	...
	function _is_order_paid() {
		global $payzenMultiResponse;

		$orderId = $payzenMultiResponse->get('order_id');
		$customerId = $payzenMultiResponse->get('cust_id');
		$transId = $payzenMultiResponse->get('trans_id');
